...
public class DES 
{
	...
	public static void main(String args[])
	{
		...
		String key=des.hexToBin(Key); // converting key into binary 
		String cipherText="";
		String Plaint="";
		for(int i=0;i<Plaintext.length()-1;i=i+8)
		{
			String plain=Plaintext.substring(i,i+8);
			plain=des.stringToBin(plain);// Converting plaintext into binary
			String cipher=des.encrypt(plain, key);
			cipherText=cipherText+des.binToString(cipher);
		}
		System.out.println("The Encrypted value is="+cipherText);
		
		for(int i=0;i<cipherText.length();i=i+8)
		{
			String cipher=des.stringToBin(cipherText.substring(i,i+8));
			Plaint=Plaint+des.binToString(des.decrypt(cipher, key));
		}
